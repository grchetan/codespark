...
-- 1. Ensure the role column and status columns exist in public.users
...
-- 2. Security Functions to Determine Caller Role
...
-- 3. Trigger: Unbreakable Root Super Admin & Role Protection
...
-- 4. Enable Row Level Security (RLS) on all relevant tables
...
-- 7. Submissions Table RLS Policies
...
-- 8. Inquiries & Requirements RLS Policies
DROP POLICY IF EXISTS "Public can submit inquiries" ON public.inquiries;
CREATE POLICY "Public can submit inquiries" 
  ON public.inquiries FOR INSERT 
  WITH CHECK (true);

DROP POLICY IF EXISTS "Staff can manage inquiries" ON public.inquiries;
...
-- 9. Set the Super Admin account in database
UPDATE public.users 
SET role = 'superadmin', status = 'active'
WHERE email = 'chetanprajapat340@gmail.com';
