...
export interface SplashCursorHandle {
  burst: (rect: DOMRect) => void;
}

// React Bits SplashCursor — rainbow GPU fluid simulation with lighting/shading.
// Pavel Dobryakov style WebGL Navier–Stokes solver.

const SplashCursor = forwardRef<SplashCursorHandle, SplashCursorProps>(
  function SplashCursor(
    ...
    const burstRef = useRef<((rect: DOMRect) => void) | null>(null);
    ...
    useEffect(() => {
      ...
      function getWebGLContext(canvas: HTMLCanvasElement) {
        ...
        if (isWebGL2) {
          glAny.getExtension('EXT_color_buffer_float');
          supportLinearFiltering = glAny.getExtension(
            'OES_texture_float_linear',
          );
        } else {
          halfFloat = glAny.getExtension('OES_texture_half_float');
          supportLinearFiltering = glAny.getExtension(
            'OES_texture_half_float_linear',
          );
        }
        glAny.clearColor(0.0, 0.0, 0.0, 1.0);
      ...
      function updateColors(dt: number) {
        ...
        if (colorUpdateTimer >= 1) {
          ...
          pointers.forEach((p) => {
            p.color = config.RAINBOW_MODE
              ? generateColor()
              : normalizeColor(hexToRgb(config.COLOR));
          });
        }
      }
      ...
      function updatePointerDownData(
        ...
        pointer.deltaX = 0;
        pointer.deltaY = 0;
        pointer.color = config.RAINBOW_MODE
          ? generateColor()
          : normalizeColor(hexToRgb(config.COLOR));
      }
      ...
      // expose burst: shoots fluid outward from a DOM element's perimeter
      burstRef.current = (rect: DOMRect) => {
        ...
        for (let i = 0; i < points; i++) {
          const angle = (i / points) * Math.PI * 2 + (Math.random() - 0.5) * 0.2;
          const ex = cx + Math.cos(angle) * (radiusX + 10);
          const ey = cy + Math.sin(angle) * (radiusY + 10);
          const posX = scaleByPixelRatio(ex - canvasRect.left);
          const posY = scaleByPixelRatio(ey - canvasRect.top);
          const tx = posX / canvas.width;
          const ty = 1.0 - posY / canvas.height;
          const force = 12000 + Math.random() * 4000;
          ...
          color.g *= 7.5;
          color.b *= 7.5;
          splat(tx, ty, dx, dy, color);
        }
      };

      function onMouseDown(e: MouseEvent) {
        const rect = canvas.getBoundingClientRect();
        const isContained = rect.width < window.innerWidth || rect.height < window.innerHeight;
        if (isContained) {
      ...
      function onFirstMouseMove(e: MouseEvent) {
        ...
        const color = config.RAINBOW_MODE
          ? generateColor()
          : normalizeColor(hexToRgb(config.COLOR));
        updateFrame();
        updatePointerDownData(pointer, -1, posX, posY);
      ...
      function onTouchStart(e: TouchEvent) {
        ...
        for (let i = 0; i < touches.length; i++) {
          if (isContained) {
            if (touches[i].clientX < rect.left || touches[i].clientX > rect.right || touches[i].clientY < rect.top || touches[i].clientY > rect.bottom) {
              continue;
            }
          }
          const posX = scaleByPixelRatio(
